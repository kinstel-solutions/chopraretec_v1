            ...
            {certifications.items.map((cert, index) => (
              <div key={index} className={`flex flex-col md:flex-row items-center gap-16 ${index % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className="flex-1 w-full">
                    <div className={`relative w-full ${cert.title.includes('ESG') ? 'aspect-[1/1]' : cert.title.includes('D&B') ? 'aspect-[1.414/1]' : 'aspect-[1/1.414]'} bg-white shadow-2xl overflow-hidden rounded-sm border border-gray-200`}>
                        {cert.thumbnail ? (
                           <Image 
                              src={cert.thumbnail} 
                              alt={cert.title}
